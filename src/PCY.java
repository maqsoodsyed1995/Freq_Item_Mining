...
class PCY{
	  ...
	  public PCY(int mod) throws IOException
	  {
		  ...
		  //ystem.out.println(modValue);
		    ...
		    FileReader fr=new FileReader("C:\\Users\\admin\\Downloads\\Freq_Item_Mining\\src\\input.txt");	
	  ...
	  private static void print(int[] combination, int[] elements){

			String output = "";
			int sum[]=new int[elements.length];
			for(int z = 0 ; z < combination.length;z++){
				sum [z]= elements[combination[z]];
			}
			int Hashvalue=Modfunc(sum,5);
...
//		        	 System.out.println();
		 ...
		 int checkCommonElements(Freq_objects one, Freq_objects another) 
		 {
		  ...
		  {
			  BufferedWriter writer = new BufferedWriter(new FileWriter("C:\\Users\\admin\\Downloads\\Freq_Item_Mining\\src\\output.txt", true));
